...
def add_ads_parser(direct_commands: Any) -> None:
    ...
    parser = direct_commands.add_parser(
        "ads",
        help="Объявления Яндекс Директ по идентификатору фразы",
        description="Список объявлений Яндекс Директ по идентификатору фразы",
        epilog=DIRECT_ADS_FIELDS,
    )
    parser.add_argument("--kid", type=int, required=True, help="Идентификатор фразы")
    add_query_options(parser)
...
def invoke(client: Any, args: argparse.Namespace) -> Any:
    ...
    if args.action == "direct.ads":
        return direct.retrieve_ads(kid=args.kid, **query)
